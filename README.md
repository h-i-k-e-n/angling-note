...
### 釣りの一連を最高に楽しむために、そして把握しやすくするために道具の管理から予定の作成、予定に伴うチェックリストの作成、釣果の詳細を備忘録として残せる。<br>
...
登録したアイテムは登録が新しい順に10個表示されます。登録したアイテムの総数、総額、総重量が表示されます。ヘッダーにはそれぞれ、アイテム登録、カテゴリ登録、アイテム一覧、プラン一覧、リスト一覧、釣果写真一覧ページに移動できます。
...
キーワード検索が可能でアイテム名、タイトル名、メモの内容で検索することも可能となっています。
...
# 課題や今後実装したい機能
- アイテム登録時、カテゴリー登録の欄を増やすか検討中。
- カテゴリー別表示に合わせてアイテム総数、金額、重さの表示もカテゴリー別表示ができるようにする。
- 画像登録の際、javascriptを使用し使用する画像を表示する。
- カテゴリーに削除する機能を追加するか検討中。
- 各ページのエラーメッセージ表示の際、対象の入力欄付近に表示させる。（現在は一番上に一括表示）
- チェックリストの詳細ページを入力データを保持しながら見ることができるチェックボックス表示。
- 作成したチェックボックス内の情報で総重量を表示。
- プランのスケジュールの詳細を追記した際、入力した順序に関係なく入力した時間順に表示する。
- アイテムの使用中、不使用、lostが一覧でわかるように表示

スマートウォッチに連携できるように設計できれば、チェックリストから現在使用しているルアーを選択し使用時間の記録や釣れた時の脈拍などデータを取ることでどれほどエキサイティングしているか見ることができる。
